...
@section('content')
    <div class="row">
        <div class="col-md-12">
            ...
            @endif
            <form method="post" action="{{route('conference.update', $conference->id)}}" id="js-form" enctype="multipart/form-data">
                @method('put')
                @csrf
                <div class="card ">
                    <div class="card-header card-header-rose card-header-icon">
                        ...
                        <h4 class="card-title">Изменить конференцию</h4>
                    </div>
                    <div class="card-body ">
                        <div class="form-group bmd-form-group">
                            <label for="exampleTitle" class="bmd-label-floating">Наименование</label>
                            <input type="text" name="title" class="form-control" id="exampleTitle" value="{{$conference->title}}">
                        </div>
                        <div class="form-group bmd-form-group">
                            <label for="exampleContent">Описание</label>
                            <textarea name="content" id="exampleContent">{{$conference->content}}</textarea>
                        </div>
                        <div class="form-group bmd-form-group">
                            <label for="exampleAdv">Преимущества</label>
                            <textarea name="advantages" id="exampleAdv">{{$conference->advantages}}</textarea>
                        </div>
                        <div class="form-group bmd-form-group">
                            <label for="exampleZoom" class="bmd-label-floating">Идентификационный номер конференции Zoom</label>
                            <input type="text" name="zoomId" class="form-control" id="exampleZoom" value="{{$conference->zoomId}}">
                        </div>
                        <div class="form-group bmd-form-group">
                            <label for="examplePassword" class="bmd-label-floating">Пароль конференции</label>
                            <input type="text" name="password" class="form-control" id="examplePassword" value="{{$conference->password}}">
                        </div>
                        <div class="form-group bmd-form-group">
                            <label for="examplePrice" class="bmd-label-floating">Цена</label>
                            <input type="text" name="price" class="form-control" id="examplePrice" value="{{$conference->price}}">
                        </div>
                        <div class="form-group bmd-form-group">
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="card ">
                                        ...
                                        <div class="card-body ">
                                            <div class="form-group bmd-form-group is-filled">
                                                <input type="text" name="start" class="form-control datetimepicker" value="{{$conference->start}}">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="card ">
                                        ...
                                        <div class="card-body ">
                                            <div class="form-group bmd-form-group is-filled">
                                                <input type="text" name="end" class="form-control datetimepicker" value="{{$conference->end}}">
                                            </div>
                                        </div>
                                    </div>
                                </div>
